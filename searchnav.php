...
.blurred {
    filter: blur(20px); /* Adjust the blur strength as needed */
        ...
        $image2 = $row['image2'];
        $image3 = $row['image3'];
        $qi = $row['question_image'];
 echo "<div class = 'ontainer'>";
        ...
        if (!empty($row["question_image"])) {
            echo "<div><img src='data:image/jpeg;base64," . base64_encode($qi) . "' /></div>";
            echo '<br>';
        }
        echo '<hr>';
       echo "<h3><b><u><font size='+4'>Answer:</font></u></b></h3>";
                echo '<br>';
                if (!empty($row["answer"])) {
                            echo "<b><font size ='+2'><p>" . $row["answer"] . "</p></font></b>";
                        }
        // Display images
        if (!empty($row["image1"])) {
 ...
 echo "</div>";        
                        echo '
<hr style="border: 10px; height: 10px; background-image: linear-gradient(to right, #ff8c00, #ff0080, #8a2be2, #4b0082);">';
                    }
                }  else {
